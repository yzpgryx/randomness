...
func worker_1E6(jobs <-chan string, out chan<- *R) {
	for filename := range jobs {
		...
		log.Printf("[%s] 重叠子序列检测 m=3 P1: %.5f P2: %.5f Q1: %.5f Q2: %.5f", filename, p1, p2, p1, p2)
		p1, p2, q1, q2 = randomness.OverlappingTemplateMatchingProto(bits, 5)
		PArr = append(PArr, p1, p2)
		QArr = append(QArr, q1, q2)
		log.Printf("[%s] 重叠子序列检测 m=5 P1: %.5f P2: %.5f Q1: %.5f Q2: %.5f", filename, p1, p2, p1, p2)
		...
		PArr = append(PArr, p)
		QArr = append(QArr, q)
		log.Printf("[%s] 线性复杂度检测 m=1000 P: %.5f Q: %.5f", filename, p, q)
